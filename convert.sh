...
inotifywait -m -q -e moved_to --format %f /home/ubuntu/Dropbox/PhD/Markdown/ | while IFS= read -r file; do
  ...
  pandoc "$path"/Markdown/"$filename".markdown -s -c basic.css -o /var/www/html/"$filename".html --smart
  pandoc "$path"/Markdown/"$filename".markdown -s -o "$path"/PDF/"$filename".pdf
  if [ "$filename"=="cv" ]
  then
    git pull
    pandoc "$path"/Markdown/cv.markdown -s -c cv.css -o /home/ubuntu/ltrgoddard.github.com/index.html --smart
    git add index.html
